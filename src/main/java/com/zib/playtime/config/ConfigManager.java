    ...
    private void migrateConfig() {
        ...
        if (config.configVersion < 2) {
            ...
            if (config.milestones == null) {
                config.milestones = new PlaytimeConfig.MilestoneSettings();
            }
            if (config.milestones.list == null) {
                config.milestones.list = new java.util.ArrayList<>();
            }
            if (config.restReminder == null) {
                config.restReminder = new PlaytimeConfig.RestReminderSettings();
            }
            if (config.integrations == null) {
                config.integrations = new PlaytimeConfig.IntegrationSettings();
            }
            ...
            if (config.messages.permissionGranted == null) {
                config.messages.permissionGranted = "&a[Playtime] &7You've been granted permission: &e%permission%";
            }
            if (config.messages.groupAssigned == null) {
                config.messages.groupAssigned = "&a[Playtime] &7You've been added to the &e%group% &7group!";
            }
            if (config.messages.restReminderMsg == null) {
                config.messages.restReminderMsg = "&e[Playtime] &7You've been playing for &e%session_time%&7. Consider taking a break!";
            }
